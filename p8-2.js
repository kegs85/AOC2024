    ...
    for(c of a[1]) {
      for (d of a[1]) {
        ...
        if (lineDiff != 0 || columnDiff != 0) {
          ...
          while(0 <= Number(tempPos[0]) && Number(tempPos[0]) < lineMax && 0 <= Number(tempPos[1]) && Number(tempPos[1]) < colMax) {
            if(!anti.has(tempPos[0]+ ':' + tempPos[1])) {
                output++;
                anti.add(tempPos[0]+ ':' + tempPos[1]);
              }
              tempPos = [tempPos[0] + Number(lineDiff), tempPos[1] + Number(columnDiff)];
          }
        }
      }
